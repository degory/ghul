...
- Array literals like `[a, b]` produce `T[]`, which doesn't auto-convert to `Iterable[T]` in argument positions. Either build a `LIST[T]()` and `.add(...)` to it, or write a helper that takes named parameters (e.g. `types_of(a, b)`).
