...
`#SEMANTICTOKENS#` shares the same shape: given a file path it dumps every recorded symbol use as tab separated `startLine  startColumn  endLine  endColumn  tokenType  modifiers` rows, where `tokenType` is one of VS Code's LSP semantic-token names (`class`, `interface`, `struct`, `enum`, `enumMember`, `typeParameter`, `namespace`, `method`, `function`, `property`, `variable`, `parameter`) and `modifiers` is a possibly-empty comma-separated list (currently just `static`). Symbol uses whose kind has no LSP mapping contribute no row. Like `#HOVERMAP#` it never recompiles; the caller drives an `#EDIT#` (and/or `#COMPILE#`) first. Powers VS Code's semantic-token provider for ghūl.
