...
for I in 1 2 3 ; do
    if docker run --env CHANGELOG_GITHUB_TOKEN --rm -v `pwd`:/usr/local/src/your-app ferrarimarco/github-changelog-generator -u degory -p ghul --future-release $VERSION ; then
        break;
    fi
    sleep 5
done
