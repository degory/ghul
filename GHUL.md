...
Two warnings police the choice of style, and both are off by default — either style compiles silently. `--warn missing-semicolon` reports every inferred boundary, for a project that wants its terminators written out; `--warn redundant-semicolon` reports a written `;` the line break would infer anyway — every end-of-line terminator except one whose next line opens with a closing or clause keyword or `)`, where the terminator is semantic (it separates a discarded statement from a tail value; see below), and one after a dangling operator, which ends an expression the line break would otherwise continue. Once enabled with `--warn`, a slug responds to `--warn-as-error` and the other severity flags like any other.
