...
A type followed by `?` is an *optional* type — a value of `T?` may be present or it may be absent. The same type without the `?` is non-optional: a value is always expected to be there.
...
The postfix `?` operator tests whether an optional holds a value, and the postfix `!` operator reads the value out. `!` is rarely needed directly: a plain `if x?` narrows `x` to its non-optional form for the rest of the branch, and `if let` tests an optional and reads its value into a variable in one step.
...
Optional types cover reference and value types alike. An optional value type is backed by `OPTION[T]`, the .NET `Nullable[T]`, so a present value is constructed explicitly — `let here: int? = OPTION[int](42);`.

Assigning `null`, or an optional the compiler cannot see is present, where a non-optional type is expected produces a warning; the warning clears once the value is known to be present — inside an `if x?` test, inside an `if let`, or through an explicit `!`.
...
Every argument must have an explicit type — the compiler does not infer argument types. The return type may be omitted when it is `void`, and on a `=>` body it can be omitted and inferred. A block body uses `return` to produce a value; reaching the end of a non-void function without a `return` returns the default value of the return type.
...
ghūl compiles to .NET IL and can consume most types in any .NET assembly. .NET names are mapped to ghūl conventions: method, property, and field names become `snake_case`; enum names and members become `MACRO_CASE`; class, struct, and trait names are left as they are, keeping any generic arity suffix (`` KeyValuePair`2 ``). The namespace `System.Collections.Generic` maps to `Collections` and `System.IO` to `IO`, and some common types are remapped — `System.Console` is `IO.Std`, `System.Type` is `System.Type2`, `IReadOnlyList<T>` is `Collections.List[T]`, `IEnumerable<T>` is `Collections.Iterable[T]`. The dotnet-integration page has the full mapping table.
