...
- A line that opens with `.` or `|>` continues the expression above it, so member chains and thread-first chains wrap.
...
A terminator carries no meaning beyond the boundary: whether a body's final statement ends with `;` never changes what the body returns, since the tail is judged by its type (see functions below).
